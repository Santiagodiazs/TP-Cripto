...
# Distribuir
assert_ok "distribución k=7 n=8" \
    "$SSS" -d -secret "$FIXTURES/secret_300.bmp" -k 7 -n 8 -dir "$WORK" \
    || fail=$((fail+1))

# Recuperar (el binario debe encontrar k portadoras con sombras válidas)
assert_ok "recuperación k=7" \
    "$SSS" -r -secret "$WORK/recovered.bmp" -k 7 -dir "$WORK" \
    || fail=$((fail+1))
