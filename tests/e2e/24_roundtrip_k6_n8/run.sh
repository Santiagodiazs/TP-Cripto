...
# Distribuir
assert_ok "distribución k=6 n=8" \
    "$SSS" -d -secret "$FIXTURES/secret_300.bmp" -k 6 -n 8 -dir "$WORK" \
    || fail=$((fail+1))

# Recuperar (el binario debe encontrar k portadoras con sombras válidas)
assert_ok "recuperación k=6" \
    "$SSS" -r -secret "$WORK/recovered.bmp" -k 6 -dir "$WORK" \
    || fail=$((fail+1))
