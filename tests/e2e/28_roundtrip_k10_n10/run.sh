...
# Distribuir
assert_ok "distribución k=10 n=10" \
    "$SSS" -d -secret "$FIXTURES/secret_300.bmp" -k 10 -n 10 -dir "$WORK" \
    || fail=$((fail+1))

# Recuperar (el binario debe encontrar k portadoras con sombras válidas)
assert_ok "recuperación k=10" \
    "$SSS" -r -secret "$WORK/recovered.bmp" -k 10 -dir "$WORK" \
    || fail=$((fail+1))
